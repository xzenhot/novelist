...
A catalog of every thematic category set available in `stereotypes/poetry/themes/`. Each theme file defines the philosophical lenses that give chapters their spine — the categories the writer agent cycles through when assigning a lens to each term from the index.
...
1. Create a new theme file in `stereotypes/poetry/themes/` (e.g. `rumi.md`, `tagore.md`, `nietzsche.md`).

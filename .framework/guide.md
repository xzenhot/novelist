...
> **Shared rule:** the filter concept is defined in `.framework/rules/filters.md` and applies to all workflows. Every book pipeline passes its material through a set of filters (seeds → research → workshop → chapters, or the poetry equivalent), each owning a folder in the pipeline.

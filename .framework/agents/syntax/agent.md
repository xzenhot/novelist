...
Record the result in `.space/pipeline/<bookname>/filters/syntax/syntax.json` (per-chapter records if the workflow creates them), and record the syntax sample selection in the chapter's model file `.space/pipeline/<bookname>/chapters/<n>/model.json` (a `syntax` object with the `form` and `sample` fields).

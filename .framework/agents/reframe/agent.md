...
**You do not generate new chapters from topics.** You take finished chapter text and transform it. The writer agent (`pipeline.md`) produces the base chapters; you reframe them.
...
> **Shared rule:** the filter concept is defined in `.framework/rules/filters.md` and applies to all workflows. Reframing is itself a filter pass — it applies the human's `override.md` (the human-in-the-loop filter) and the quality review to already-written chapters.

...
You are the **human-in-the-loop** agent of the pipeline. Your task is to read the book's **master prompt** — `.space/pipeline/<bookname>/override.txt` — and record the override mandate in each chapter's `model.json`, so the prose the writing path later composes conforms to the book's identity, premise, form, language, and style mandate.
...
3. `.space/pipeline/<bookname>/filters/override/filter.md` — optional human instructions (see below).
...
- **Chapter models** — update `.space/pipeline/<bookname>/chapters/<n>/model.json` with the `override` result for each chapter (`status: "passed_through"` when no mandate was present). `chapter.md` is not written here; it is authored only by the writing/authorship agents, which read this recorded `override` mandate.

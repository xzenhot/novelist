---
name: postlayout
description: Post-scaffold master-prompt generator. Runs after the scaffold agent has built a pipeline. Reads the backlog master prompt at .space/backlog/epic/<bookname>/override.txt, then derives a dynamic, pipeline-specific master prompt and writes it to .space/pipeline/book_<bookname>/override.txt. Does not scaffold, filter, or write chapters.
tools: ["read", "write"]
---
...
- Output: `.space/pipeline/book_<bookname>/override.txt`
...
Write a **dynamic master prompt** to `.space/pipeline/book_<bookname>/override.txt`. It is the pipeline's operative writing mandate, re-derived from the backlog idea against the concrete pipeline state. It must include:
...
- **Do not overwrite a human-edited file.** If `.space/pipeline/book_<bookname>/override.txt` already exists and contains human edits, do not overwrite it; report that it exists and leave it. Otherwise regenerate it.
...
1. The absolute path of the written file: `.space/pipeline/book_<bookname>/override.txt`.
...
- The output file is `.space/pipeline/book_<bookname>/override.txt` — distinct from the novel-only `masterprompt.md` planning artifact and from the backlog `override.txt`.

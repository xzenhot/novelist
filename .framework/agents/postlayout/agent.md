---
name: postlayout
description: Post-scaffold master-prompt generator. Runs after the scaffold agent has built a pipeline. Reads the backlog master prompt at .space/backlog/epic/<bookname>/masterprompt.txt, then derives a dynamic, pipeline-specific master prompt and writes it to .space/pipeline/book_<bookname>/masterprompt.txt. Does not scaffold, filter, or write chapters.
tools: ["read", "write"]
---
...
- Source idea: `.space/backlog/epic/<bookname>/masterprompt.txt` (the backlog identity/mandate record)
- Pipeline state: `.space/pipeline/book_<bookname>/model.json`, `.space/pipeline/book_<bookname>/bookseed.txt` (poetry) or `.space/pipeline/book_<bookname>/book.json` (novel)
- Output: `.space/pipeline/book_<bookname>/masterprompt.txt`
...
1. The scaffold agent has finished building `.space/pipeline/book_<bookname>/`.
2. The pipeline `model.json` exists and declares the `form`.
3. The backlog master prompt exists at `.space/backlog/epic/<bookname>/masterprompt.txt`.
...
1. `.space/backlog/epic/<bookname>/masterprompt.txt` — the book's static identity and mandate (the seed idea).
2. `.space/pipeline/book_<bookname>/model.json` — the resolved form, language, register, quality, themes, reference, signature, sacred vocabulary, translation guide, gist, and book summary.
3. `.space/pipeline/book_<bookname>/bookseed.txt` (poetry) — the concrete topic list; or `.space/pipeline/book_<bookname>/book.json` (novel) — the chapter plan.

## What to Produce

Write a **dynamic master prompt** to `.space/pipeline/book_<bookname>/masterprompt.txt`. It is the pipeline's operative writing mandate, re-derived from the backlog idea against the concrete pipeline state. It must include:
...
1. The absolute path of the written file: `.space/pipeline/book_<bookname>/masterprompt.txt`.
...
- The output file is `.space/pipeline/book_<bookname>/masterprompt.txt` — distinct from the novel-only `masterprompt.md` planning artifact and from the backlog `masterprompt.txt`.

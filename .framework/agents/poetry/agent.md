...
7. **Flatten, then prompt.** Before the selected skill writes, flatten the (possibly refreshed) chapter `model.json` into `context.md` and build the writer prompt from that `context.md` and the pipeline `style.md` (see *Flatten Model → Context → Prompt*). Pass the flattened `context.md` content and the built prompt to the selected skill — never the raw `model.json`.
